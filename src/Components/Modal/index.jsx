    ...
    <div className="modal-main__backdrop">
      <Grid container spacing={0} className={`modal-main__content ${customSize}`}>
        <Grid container spacing={0} className="modal-main_header">
          <Grid item xs={10} className="modal-main_title">{title}</Grid>
          <Grid item xs={2} className="modal-main_close" onClick={() => updateModal({ payload: { status: false, element: <div /> } })}>X</Grid>
        </Grid>
        <Grid
          container
          spacing={0}
